...
async function initCarousel(id) {
  ...
  function setClassToPrevNextSlide() {
    const slideActive = myCarousel.querySelector(
    ...
    new Promise((resolve) => {
      ...
      if (event.direction === "left") {
        if (different < 2 || event.from === lengElements - 1) {
          const indicatorFirst = myCarousel.querySelector(
            ".carousel-indicators > button"
          );
          addToEnd(indicatorFirst);
        } else {
          if (event.from === 0 && different === lengElements - 1) {
            const indicatorLast = myCarousel.querySelector(
        ...
        if (different < 2 || event.to === lengElements - 1) {
          ...
          if (
            event.from === lengElements - 1 &&
            different === lengElements - 1
          ) {
            const indicatorFirst = myCarousel.querySelector(
              ".carousel-indicators > button"
            );
  ...
  async function toggleFavoriteBrand() {
    ...
    if (userToken) {
      const resp = await fetch(
        "https://64a990b78b9afaf4844ad897.mockapi.io/favoritebrand"
      );
      const result = await resp.json();
...
function imgSetAttributes() {
  ...
  function imgProductSetAttributes() {
    let productPics = document.querySelectorAll(
    ...
    for (let i = 0; i < productPics.length; i++) {
      if (productPics[i].classList.contains("active")) {
        ...
        productPics[i]
          .querySelector("img")
          .setAttribute("data-bs-target", "#openFullImg");
      } else {
        productPics[i]
          .querySelector("img")
          .removeAttribute("data-bs-toggle", "modal");
        productPics[i]
          .querySelector("img")
          .removeAttribute("data-bs-target", "#openFullImg");
      }
    }
  }
...
function toggleFavoriteBrand() {
  ...
  favoriteList.forEach((item) => {
    item.addEventListener("click", (elem) => {
      ...
      } else elem.target.classList.remove("active");
    });
  });
}
toggleFavoriteBrand();
